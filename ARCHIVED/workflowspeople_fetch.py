...
def logging() -> None:
    ...
    print(f'people fetch = {PEOPLE}, campus fetch = {CAMPUSES}, workflow fetch = {WORKFLOWS}')
    ...
    if WORKFLOWS == True:
        ...
        def retrieveWorkflowPeople(workflow_ID: int, batchsize = 100) -> dict:
            ...
            for attempt in range(retries):
                ...
                if cardresponse.status_code == 200:
                    ...
                    print(f"[Workflow People ({workflow_ID})]: Rate limit exceeded. Waiting {wait_time} seconds before retrying...")
            ...
            for attempt in range(retries):
                ...
                if response.status_code == 200:
                    ...
                    for o in range(L)[::-1]:
                        ...
                        step_tracker = {}   
                        # print(indexer, person['data'][o]['attributes']['comment'])

                        #extract comment
                        comment = person['data'][o]['attributes']['comment']

                        #determine when card was created (person signed up)
                        if comment.startswith("created this card"):
                        ...
                        if comment.startswith("sent this back"):
                            #if a person was moved back a step, only consider the time once they have completed this step
                            indexer += -1
                            continue


                        #determine when step was completed or skipped (person moved forward)
                        if comment.startswith("completed the step") or comment.startswith("skipped the step"):
                            try:
                                #get time and sequence name 
                                time_init = person['data'][o]['attributes']['created_at']
                                seq_name = workflow_stepslist[indexer]['name']
                                ...
                                indexer += -1
                                print(f"(ID: {workflow_id}, CARD: {workflow_card}) Indexing error in fetching workflow history. KeyError raised. Possible duplicate comments. Review:")
                                print(f"{person['data']}")
                                continue
                        ...
                        if comment.startswith("completed this card"):
                    ...
                    if retry_after:
                        ...
                        wait_time = 2 ** attempt  # Exponential backoff if Retry-After is not provided
                    print(f"[Workflow History ({workflow_id})]: Rate limit exceeded. Waiting {wait_time} seconds before retrying...")
                    time.sleep(wait_time)
        ...
        def writeWorkflowHistCSV(
            ...
            try:
                ...
                print(f'I/O Error while writing CSV ({workflow_id}).')
